...
export const computeMultiSegmentBezierPath = (
  ...
  if (points.length === 2) {
    ...
    const p1 = points[1]!;
    const dx = p1.x - p0.x;
    return `M ${p0.x} ${p0.y} C ${p0.x + dx * curvature} ${p0.y}, ${p1.x - dx * curvature} ${p1.y}, ${p1.x} ${p1.y}`;
  }
  ...
  for (let i = 0; i < points.length - 1; i++) {
    ...
    const cp1x = p1.x + (p2.x - p0.x) * (curvature * 0.5);
    const cp1y = p1.y + (p2.y - p0.y) * (curvature * 0.5);
    const cp2x = p2.x - (p3.x - p1.x) * (curvature * 0.5);
    const cp2y = p2.y - (p3.y - p1.y) * (curvature * 0.5);
...
export const computeWaypointOrthogonalPath = (
  points: Array<{ x: number; y: number }>
): string => {
  if (points.length < 2) return '';
  let d = `M ${points[0]!.x} ${points[0]!.y}`;
