...
export const createSchemaTabs = function(instanceId: string): SchemaTabsResult {
  ...
  addBtn.addEventListener('click', () => {
    ...
    // Fire intent only — ID is generated by the reducer (standalone) or by the
    // MCP server (when a dispatch hook is registered by the canvas page).
    store.dispatch({ type: 'schema-create-auto', name });
  });
