const fs = require('fs');
const path = 'e:/Sources/atomos-monorepo/packages/atomos-prime/src/features/accordion/create-accordion.ts';
const content = `import type { AccordionProps, AccordionResult } from './types/accordion.types.js';
...
export const createAccordion = function(props: AccordionProps): AccordionResult {
  ...
  element.className = \`border border-gray-200 dark:border-slate-800 rounded-lg overflow-hidden shrink-0 \${props.className || ''}\`;

  // Header (clickable trigger)
  const header = document.createElement('button');
  header.className = 'w-full px-4 py-3 text-left bg-gray-50 hover:bg-gray-100 dark:bg-slate-950 dark:hover:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-inset flex justify-between items-center';

  const titleSpan = document.createElement('span');
  titleSpan.className = 'font-medium text-gray-900 dark:text-slate-200';
  ...
  chevron.className = 'transform transition-transform duration-200 text-slate-500 flex-shrink-0';
  chevron.innerHTML = '▼';
  ...
  contentInner.className = 'px-4 py-3 bg-white dark:bg-transparent';
  ...
  const updateDisabled = (disabled: boolean) => {
    ...
    if (disabled) {
      ...
      header.classList.remove('hover:bg-gray-100', 'dark:hover:bg-slate-900');
    } else {
      header.classList.remove('opacity-50', 'cursor-not-allowed');
      header.classList.add('hover:bg-gray-100', 'dark:hover:bg-slate-900');
    }
  };
