...
export const createAccordion = function(props: AccordionProps): AccordionResult {
  ...
  element.className = \`border border-gray-200 border-slate-800 rounded-lg overflow-hidden shrink-0 \${props.className || ''}\`;

  // Header (clickable trigger)
  const header = document.createElement('button');
  header.className = 'w-full px-4 py-3 text-left bg-gray-50 hover:bg-gray-100 bg-slate-950 hover:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-inset flex justify-between items-center';

  const titleSpan = document.createElement('span');
  titleSpan.className = 'font-medium text-gray-900 text-slate-200';
  ...
  contentInner.className = 'px-4 py-3 bg-transparent';
  ...
  const updateDisabled = (disabled: boolean) => {
    ...
    if (disabled) {
      ...
      header.classList.remove('hover:bg-gray-100', 'hover:bg-slate-900');
    } else {
      header.classList.remove('opacity-50', 'cursor-not-allowed');
      header.classList.add('hover:bg-gray-100', 'hover:bg-slate-900');
    }
  };
